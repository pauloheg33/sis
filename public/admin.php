...
$users = $pdo->query('SELECT users.name, users.email, users.role, schools.name AS school FROM users LEFT JOIN schools ON users.school_id = schools.id')->fetchAll();
...
<?php foreach ($schools as $school): ?>
        <li><?= htmlspecialchars($school['name']) ?></li>
<?php endforeach; ?>
    </ul>
...
<?php foreach ($users as $user): ?>
        <li><?= htmlspecialchars($user['name']) ?> (<?= $user['role'] ?>) - <?= htmlspecialchars($user['email']) ?> - <?= htmlspecialchars($user['school']) ?></li>
<?php endforeach; ?>
    </ul>
